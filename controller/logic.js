...
const Create_New_Post = (req, res) => {
    try {
        if (fs.existsSync('../jsonFile/posts.json')) {
            const allposts = JSON.parse(fs.readFileSync('.'))
            allposts.push(req.body)
            fs.writeFileSync('../jsonFile/posts.json', JSON.stringify(allposts, null, 4))
            console.log(allposts);
            res.json({ posts: allposts })
        } else {
            fs.writeFileSync('../jsonFile/posts.json', JSON.stringify([req.body], null, 4))
        }
    } catch (error) {
...
const List_all_Posts = (req, res) => {
    try {
        const Posts = JSON.parse(fs.readFileSync('../jsonFile/posts.json'));
        console.log(Posts);
        res.json({ Posts })
    } catch (error) {
        res.send(error.message)
...
const Update_Post = (req, res) => {
    try {
        const AllPost = JSON.parse(fs.readFileSync('../jsonFile/posts.json'));
        ...
        fs.writeFileSync('../jsonFile/posts.json', JSON.stringify(AllPost, null, 4))
        res.json({ AllPost })
    } catch (error) {
        res.send(error.message)
    }
...
const Delet_Post = (req, res) => {
    try {
        ...
        fs.writeFileSync('../jsonFile/posts.json', JSON.stringify(updatedArray, null, 4))
...
const Search_Post = (req, res) => {
    ...
    try {
        ...
        const All_Post = JSON.parse(fs.readFileSync('../jsonFile/posts.json'));
...
const List_single_Post_corrosponding_comments = (req, res) => {
    try {
        const AllPost_comment = JSON.parse(fs.readFileSync('../jsonFile/comments.json'));
...
const List_Post_with_given_userid_and_fetch_comments = (req, res) => {
    try {
        ...
        const AllPost_comment1 = JSON.parse(fs.readFileSync('../jsonFile/comments.json'));
